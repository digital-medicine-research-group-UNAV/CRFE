import numpy as np
import scipy as sp
import pandas as pd
import os

from sklearn.preprocessing import minmax_scale
from sklearn.impute import KNNImputer
from scipy import stats
from collections import Counter

import warnings
warnings.simplefilter(action='ignore', category=FutureWarning)



def crea_dataset(lista):

    dataset = []
    for sublista in lista:

        b = []
        i = 0
        flag = False
        for ele in sublista:
            
            if flag == True:
                flag = False
                continue

            if ele != ",":
                
                # esta condición se podria generalizar para longitudes arbitrarias mediante recursividad
                try:

                    if sublista[i+1] != ",": 

                        try:
                            b.append(float(ele + sublista[i+1] ))
                        except ValueError:
                            b.append(float(-1))

                        flag = True
                        i = i + 2 # mantener junto al del final del bucle

                    else:

                        try:
                            b.append(float(ele))
                        except ValueError:
                            b.append(float(-1))

                        i = i + 1

                except IndexError:  # loop is over
                    b.append(float(ele))
                    break
 
            else: 
                i = i + 1
                   
        dataset.append(b)

    return dataset

def checker(data):

    size_old = len(data[0])
    for i in range(1,len(data)):

        size_new = len(data[i])

        if size_old == size_new:
            status = "correct"
            size_old = size_new
        else:
            print(i)
            status ="wrong"

    return status


with open(r'dermatology.data') as f:
    dermatology = f.read().splitlines()


data = crea_dataset( dermatology)


print(data)

 
print(len(data))

print( checker(data))

#3  (age)  #4 (sex)  #9  (cp)  #10 (trestbps)  #12 (chol)  #16 (fbs)  #19 (restecg)   
#32 (thalach)  #38 (exang) #40 (oldpeak) #41 (slope) #44 (ca) #51 (thal) 
#informative_attributes = [2, 3, 8, 9, 11, 15, 18, 31, 37, 39, 40] # in python list start in 0 not 1


if checker(data) == "correct":
    df_ = pd.DataFrame.from_records(data)
    df_.to_csv(r'dermatology.csv', index=False)


X = data
df_for_analisis = pd.DataFrame.from_records(X)

dataset_name = "_dermatology"

list_of_index = np.arange(len(X[0])).tolist() # Empieza en 0   # Array must be a list
Y = [ele.pop(-1) for ele in X]
list_of_index.remove(len(X[0]))



#df_ = pd.DataFrame.from_records(X) 

# basic statistics
nans = [np.sum((df_for_analisis[i] == -1.0))/len(df_for_analisis) for i in range(len(df_for_analisis.mean().tolist()))]
print("Percentage of Nan by columns:")
print(np.around(nans,2).tolist())

deleted_indexes = [i for i in range(len(nans)) if nans[i] > 0.25]
print("deleted indexes: ", deleted_indexes)


X = np.delete(X, deleted_indexes, axis=1) # delete rows with more than 0.5 of Nan
X = np.delete(X, -1, axis=1)              # delete name column


[list_of_index.remove(ele) for ele in deleted_indexes]


print("list of index: ")
print(list_of_index)



X = np.array(X, dtype='float').tolist()

imputer = KNNImputer(missing_values = -1.0, n_neighbors=6)
X = imputer.fit_transform(X)


# basic statistics
print("Mean by columns (In order. After imputing):")
print(np.around(df_for_analisis.mean(),2).tolist()) #string features are avoided
print("Variance by columns (In order. After imputing):")
print(np.around(df_for_analisis.var()).tolist())
varianza = df_for_analisis.var().tolist()

"""""
min_max_index = []
binary_index = []
z_score_index = []
i = 0
for indx in list_of_index:

    z = Counter(X[:,i])
    
    if len(list(z.keys())) == 2:
        binary_index.append(indx)
        i = i+1
    else:
        if len(list(z.keys())) > 10:
            X[:,i] = stats.zscore(X[:,i])
            z_score_index.append(indx)
            i = i + 1

        else:
            X[:,i] = minmax_scale(X[:,i])
            min_max_index.append(indx)
            i = i + 1


print("Binary index: ")
print(binary_index)
print("Min Max index: ")
print(min_max_index)
print("Z score index: ")
print(z_score_index)


i = 0
for indx in list_of_index:
    X[:,i] = minmax_scale(X[:,i])
    i = i + 1


"""""

#X = stats.zscore(X, axis=1)
X = minmax_scale(X, axis=1)

np.save(r'attributes' + dataset_name + '.npy', np.array(X))
np.save(r'target' + dataset_name + '.npy', np.array(Y))
#np.save(r'index' + dataset_name + '.npy', np.array(list_of_index))
#np.save(r'inf_index.npy', np.array(informative_attributes))




